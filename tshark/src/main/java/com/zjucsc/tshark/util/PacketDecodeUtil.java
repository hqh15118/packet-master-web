...
import static com.zjucsc.common.common_config.PACKET_PROTOCOL.*;
...
public class PacketDecodeUtil {
    ...
    public static int decodeFuncode(String protocol , String str_fun_code ){
        ...
        try {
            ...
            System.err.println(String.format("exception when decode protocol [%s] fun_code [%s] ==> [PacketDecodeUtil]", protocol , str_fun_code));
        }
        return fun_code;
    }
    ...
    public static CollectorState decodeCollectorState(byte[] payload , int offset , int collectorId){
        ...
        if ((state = TsharkCommon.COLLECTOR_STATE_MAP.get(collectorId))==null){
            //还没有定义过该设备，初始化该设备状态
            TsharkCommon.COLLECTOR_STATE_MAP.put(collectorId , new CollectorState(collectorId , -1,-1,
                    A_state,B_state));
            return null;
        }
        ...
        if (A_state != state.getA_currentState() || B_state != state.getB_currentState()){
            CollectorState newState = new CollectorState(collectorId ,
                    ...
                    A_state,B_state);
            TsharkCommon.COLLECTOR_STATE_MAP.put(collectorId , newState);
            return newState;
        }
        return null;
    }
