package com.zjucsc.capture_main_distribute.handler;
    ...
    public Void handle(Object t) {
        ...
        if (protocol.startsWith("s7comm")){
            if (!layer.tcp_flags_ack[0].equals("")){
                res =  ArtDecodeCommon.artDecodeEntry(CaptureCommon.getGlobalArtMap(),tcpPayload,"s7comm",1);
            }else{
                res =  ArtDecodeCommon.artDecodeEntry(CaptureCommon.getGlobalArtMap(),layer.rawData,"s7comm",0);
            }
        }else if (protocol.equals(PACKET_PROTOCOL.MODBUS)){
            res = ArtDecodeCommon.artDecodeEntry(CaptureCommon.getGlobalArtMap(),tcpPayload,layer.frame_protocols[0]);
        }else if (protocol.equals(PACKET_PROTOCOL.PN_IO)){
            res = ArtDecodeCommon.artDecodeEntry(CaptureCommon.getGlobalArtMap(),layer.rawData,layer.frame_protocols[0]);
